...
a finite field $\L$. Precise definitions are given below, but this means
...
Rank two Drinfeld modules enjoy remarkable similarities with elliptic curves: analogues exist of good reduction, complex
multiplication, etc. Based in part on these similarities, Drinfeld modules have recently
...
endomorphism rings~\cite{GaPa18}. Our goal is to study in detail the
...
characteristic polynomial of such a curve~\cite{schoof85}. 
...
in~\cite{Jung00} and used to study the distribution of characteristic
...
\begin{definition}\label{def:Drinfeld}
  Let \spa $ \L$ and $\gamma$ be as above.  A rank $r$ {\em Drinfeld module}
...
We will restrict our considerations to rank two Drinfeld modules. In particular, we will use the now-standard convention of writing $\varphi_x = \gamma(x)
...
\begin{example}
  ...
  module $\varphi: \F_5[x] \to \F_{625}\ang{\tau}$ by $\varphi_x = \zeta + \tau + \tau^2$, so that $(g,\Delta)=(1,1)$.
\end{example}
...
an endomorphism. The following key theorem~\cite[Cor. 3.4]{GEKELE1991187}
...
$A = 3x^2+x+3$ and $B =x^4+4x^2+4x+2$.
\end{example}
...
case''), where a runtime $(n^{1.5+\varepsilon} \log q + n^{1 + \epsilon} \log^2
...
Schoof's algorithm for elliptic curves. We believe this interaction has the potential to
yield further algorithms of interest, perhaps using other
``elliptic'' techniques, such as $p$-adic approaches~\cite{Satoh00} or
Harvey's amortization techniques~\cite{Harvey14}.
...
We now discuss the cost of operations in $\L$ and $\L\ang{\tau}$ with runtimes
given in bit operations. Notation ($\F_q,\L,\dots$) is as in~\ref{ssec:not}. To simplify cost
...
We could sharpen our results using the so-called
...
$\theta = (\omega+1)/2$~\cite{BrKu78}. More recently, it was shown that we can let $\theta = (\omega + 2)/3$ for generic inputs \cite{salvy_neiger_schost_gyu}, and a major result by Kedlaya and
Umans proves that we can actually take $\theta = 1+\varepsilon$, for
any $\varepsilon >
0$~\cite{Kedlaya:2011:FPF:2340436.2340448}. In practical
terms, we are not aware of an implementation of Kedlaya and Umans'
algorithm that would be competitive: for
practical purposes, $\theta$ is either $(\omega+1)/2$ (for deterministic approaches) or $(\omega+2)/3$, and $\omega$ itself is
either 3 or Strassen's $\log_2 7 \simeq 2.81$. 
%We discuss some
...
$\Theta(k)$ coefficients in $\L$, so it uses $\Theta(k n \log q)$
bits. Since $\varphi_{x^{2k}}=\varphi_{x^{k}}\varphi_{x^{k}}$
...
bit operations, which may be acceptable in practice. This also
...
polynomials, one can compute all
$U(\alpha_i)$ faster than by successive evaluation of $U$ at
...
$\omega_2 \le \omega+1$ as in {\bf 3.2.1}.
...
$k$, and let $s=\sqrt{k}$.
\begin{enumerate}
\item Commute powers of $\tau$ with the coefficients of $U$ to rewrite it as $U = U^*_{0} + \tau^s U^*_{1} + \cdots + \tau^{s(s-1)} U^*_{s-1}$.
...
\item Using Horner's scheme, for $j=1,\dots,\mu$, recover $U(\alpha_j)$ using 
...
As with elliptic curves, determining the Frobenius norm $B$
of Theorem~\ref{charpoly} is simply
done using the following result from~\cite[Th.~2.11]{frobdist}.
...
\begin{proposition}\label{prop:gek}
  One can solve Problem~\ref{pb1} using $(n^{\theta+2}\log
  q + n \log^2 q)^{1+o(1)}$ bit operations.
\end{proposition}
\begin{proof}
The algorithm is as follows.
\begin{enumerate}
\item We compute $x^q\bmod \frakp$ with $(n \log^2
  q)^{1+o(1)}$ bit operations.
\item Find $\varphi_1,\dots,\varphi_{x^n}$ in
...
Carlo algorithm to solve Problem~\ref{pb1} for odd $q$, which applies to those
...
choosing $\mu$ many $\F_q$-linear mappings $\L\to\F_q$, say ${\bm
...
\item Fix $\mu=\lfloor n^{b} \rfloor$, for some exponent $b$ to be
  determined later; choose $\mu$ many $\F_q$-linear mappings $\L\to\F_q$,
...
\item Compute ${\bm Q}$; this takes $(\mu^{\omega-1} n \log q)^{1+o(1)}$ bit operations.
\item Compute the determinant $\Gamma^*$ of ${\bm Q}$.  The cost of
  ...
  coincides with $\Gamma$. For generic $\ell_1$ and
  $\alpha_1$, the minimal polynomial of $(r_{1,1,k})_{k \ge 0}$ is $\Gamma$. If this is the case, since $\Gamma^*$ cancels
...
result.  The relevant terms are $\{K\mu n,
...
determined using an exponent $2.373$ for matrix multiplication,
which is currently unrealistic in practice).
...
The operator $\text{\sf T}$ is $\F_q$-linear but not $\L$-linear;
...
We now prove the last item in our main theorem: {\em
  there exists a Monte Carlo algorithm that solves Problem~\ref{pb1}
  in $(n^2 \log^2 q)^{1+o(1)}$ bit operations.} The runtime is now
quadratic in $\log q$, but truly quadratic in $n$, not of the form
$n^{2+\varepsilon}$. The point is that we avoid applying high
powers of the Frobenius (and thus modular composition); the
...
Theorem~\ref{charpoly} implies that for $\alpha$ as above, we have $\Phi_A(\alpha)=r$ with $ r:=\alpha + \Phi_B(\alpha) \in \L$. Using the expression of $A$ given above, this yields
...
rely, the deterministic algorithm demonstrates a
significantly higher complexity than expected. For $m=1$, as predicted by the cost 
...
%  \caption{Log-log plot of $q$ versus runtime with %$n = 50$, $m = 2$}
%\end{figure}


\begin{acks}
  We wish to thank Jason Bell and Mark Giesbrecht for their comments
  on Musleh's MMath thesis \cite{Musleh}, which is the basis of this work, and Anand
